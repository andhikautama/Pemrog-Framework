...
class GenericContainerDemo extends Component {
    render(){
        const ContactListWithGenericContainer = GenericContainer({reqUrl: 'https://demo1443058.mockable.io/users/',
    reqMethod:'GET', resName:'contacts'})(ContactLit);
    return(<div className="contactApp">
        <ContactListWithGenericContainer />
    </div>)
    }
}
const ContactLit = ({contacts}) => {
    return(
        <div>
        ...
        <ul>
            {contacts.map(
                (contact) => <li key={contact.email}>
                    <img src={contact.photo} width="1000px" height="100px" alt="presentation" />
